...
The active project is the SecretFlow proof of concept at the repo root. Start with `START_HERE.md`, then `README.md`. Runtime scripts such as `standalone_poc.py`, `coordinator.py`, and `run_2party_psi.py` live at the top level. Built-in fixtures and CSV templates are in `data/`. Reference material is in `docs/`, `systemd/` contains deployment examples, and `.env.example` contains coordinator environment defaults. Generated runtime state is written under `runs/`, `archives/`, and `poc_output/`.
