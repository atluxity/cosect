...
The active project is the SecretFlow proof of concept at the repo root. Start with `START_HERE.md`, then `README.md`. Runtime scripts such as `standalone_poc.py`, `run_2party_psi.py`, `run_2party_psi_peer.py`, and `distributed_network_poc.py` live at the top level. Built-in fixtures and CSV templates are in `data/`. Reference material is in `docs/`. Generated runtime state is written under `poc_output/` and `out/distributed_network_poc/`.
...
- `python3 distributed_network_poc.py` runs the distributed two-container demo where each party keeps its plaintext input local.
- `python3 write_peer_psi_session.py ...` writes a shared session file for the distributed mode.
...
## Testing Guidelines
Use the built-in fixtures in `data/` first. Validate local changes with `python3 -m py_compile` on modified scripts, then run either `python3 standalone_poc.py --pull` or `python3 distributed_network_poc.py` depending on whether the change affects the local or distributed flow. Keep real customer data out of git and out of sample fixtures.

## Commit & Pull Request Guidelines
Use short, imperative commit subjects that describe the user-visible change, for example `Promote SecretFlow POC to repo root`. For pull requests, explain whether the change affects the standalone POC or the distributed mode; list the commands you ran; and call out any security impact.
